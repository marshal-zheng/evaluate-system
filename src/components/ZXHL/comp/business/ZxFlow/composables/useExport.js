...
const isRefLike = (maybeRef) => Boolean(maybeRef && typeof maybeRef === 'object' && 'value' in maybeRef);
...
export function useExport(graphInstance = null) {
  ...
  const ensureGraph = () => {
    const g = getGraph();
    if (!g) {
      throw new Error('Graph instance is not available – make sure the graph has been initialised before exporting.');
    }
    return g;
  };
  ...
  const toDataURL = async (type = 'image/png', options = {}) => {
    ...
    if (typeof g[method] !== 'function') {
      throw new Error(`Graph export plugin is missing method ${method}. Did you forget to include <XFlowExport />?`);
    }
  ...
  const exportSVG = async (fileNameOrOptions, options = {}) => {
    ...
    const xml = svgString.startsWith('<?xml') ? svgString : `<?xml version="1.0" encoding="UTF-8"?>\n${svgString}`;
  ...
  const exportPDF = async (fileNameOrOptions, options = {}) => {
    ...
    const imageType = normalizedType.includes('jpeg') || normalizedType.includes('jpg') ? 'JPEG' : 'PNG';
