          ...
          <div class="dag-graph">
            ...
            >
              ...
              <DagGraphHandler 
                ...
                @context-menu-ready="onContextMenuReady"
              />
              <!-- 小地图 -->
              <XFlowMinimap 
                :width="200" 
...
import { XFlow, XFlowGraph, XFlowClipboard, XFlowState, XFlowHistory, XFlowGrid, XFlowBackground, XFlowMinimap, XFlowContextMenu } from '@/components/xflow-vue/src/components';
...
import DagGraphHandler from './components/DagGraphHandler.vue';
...
const DAGPage = defineComponent({
  ...
  setup(props) {
    ...
    const { operators, operatorsLoading, dndConfig, layout } = toRefs(props);
    ...
    // 图实例准备好的回调
    const onGraphReady = (graphInstance) => {
      graph = graphInstance;
      console.log('Graph ready:', graph);
    };

    // 右键菜单准备好的回调
    const onContextMenuReady = (menuHandler) => {
      contextMenuHandler = menuHandler;
      console.log('Context menu ready:', contextMenuHandler);
    };

    // 处理右键菜单点击
    const handleContextMenuClick = (item) => {
      if (contextMenuHandler && contextMenuHandler.handleMenuClick) {
        contextMenuHandler.handleMenuClick(item);
      }
    };
    ...
    return { 
      ...
      zoomOptions,
      contextMenu: computed(() => contextMenuHandler?.contextMenu || {}),
